...
for _ in range(args.x):
    for vpn_key, profile in vpn.items():
        ...
        if connected:
            ...
            profile_path = f'C:\\Users\\pc\\AppData\\Roaming\\Mozilla\\Firefox\\Profiles\\{profile}'  # 엣지 엣지일 경우 개발자도구 포커스 맞추기 켜야함
            command = [
                'C:\\Program Files\\Mozilla Firefox\\firefox.exe',  # 엣지
                f'-P "{profile_path}"',
            ...
            pyautogui.hotkey('ctrl', 'shift', 'i')  # 개발자도구 켜기
            time.sleep(0.2)
            ...
            for prod_ids in indices_to_search:
                ...
                for index, prod_id in enumerate(prod_ids):
                    ...
                    if len(prod_ids) > 1:
                        ...
                        pyautogui.hotkey('ctrl', 'shift', 'i')  # 새 탭에서 개발자 도구 켜기
                        # time.sleep(0.2)
                        # pyautogui.press('tab', presses=1)  # 콘솔로 포커스 맞추기
                        # time.sleep(0.5)
                        # 카탈로그 안에서 상품클릭
            ...
            pyautogui.hotkey('ctrl', 'shift', 'i')  # 개발자도구 끄기
            time.sleep(0.5)
            ...
            # 파이어폭스 종료
            print(f"{profile} 프로필 firefox 종료 중...")
